  ...
  methods: {
    async getUserList() {
      ...
      if (res.meta.status !== 200)
        return error.$message.error('获取用户列表失败！')
    ...
    async userStateChanged(userinfo) {
      ...
      if (res.meta.status !== 200) {
        ...
        return error.$message.error('更新用户状态失败！')
      }
    ...
    async removeUserById(id) {
      //弹框确认
      const confirmresult = await this.$confirm('此操作将永久删除该用户, 是否继续?', '注意', {
